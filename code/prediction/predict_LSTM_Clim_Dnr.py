# -*- coding: utf-8 -*-
"""
Created on Thu Jul  3 10:48:43 2025
...
for seed in seed_all:
    folder_name = f"/scratch/bcqp/rg727/1950_1965_Training_Testing/Predictions_NN/seed_{seed}"
...
for seed in seed_all:
    ...
    t_start = time.time()
    basin_list_train = np.loadtxt('/scratch/bcqp/rg727/1950_1965_Training_Testing/1950_1965_gauges.txt',dtype=str)    
    ...
    for basin_id in basin_list_train:   
        xy = np.loadtxt('/scratch/bcqp/rg727/1950_1965_Training_Testing/Testing/merged/'+basin_id+'.csv',delimiter=",", dtype=np.float32, skiprows=1)
    ...
    class LSTM(nn.Module):
        ...
        def forward(self,x): # x: [batch_size, seq, num_input_feature]
            ...
            out,_ = self.lstm(x, (h0,c0)) # out: batch_size, squence_length, num_hidden_neuron
...
model.load_state_dict(torch.load('/scratch/bcqp/rg727/1950_1965_Training_Testing/Testing/merged/model/lstm_nbr_ec_gl_train_seed_'+str(seed)+'_e30.pth'))
model.to(device)
...
basin_list_valid = np.loadtxt('/scratch/bcqp/rg727/combined_gauges_2.txt',dtype=str)
...
for k, basin_id in enumerate(basin_list_valid):
    xy = np.loadtxt('/scratch/bcqp/rg727/1950_1965_Training_Testing/merged_gauges_final/'+basin_id+'.csv',delimiter=",", dtype=np.float32, skiprows=1)
    y = xy[:,[3]]
...
for k, basin_id in enumerate(basin_list_valid):
    ...
    with torch.no_grad():
        ...
        #pred_obs = valid_predictions
        np.savetxt('/scratch/bcqp/rg727/1950_1965_Training_Testing/Predictions_NN/seed_'+str(seed)+'/lstm_nbr_pred_'+basin_id+'.csv', pred_obs, delimiter=',',header='pred,obs',comments='')
