# -*- coding: utf-8 -*-
"""
Created on Thu May 15 19:56:18 2025
...
for seed in seed_all:
    ...
    for basin_id in basin_list_train:   
        xy = np.loadtxt('/scratch/bcqp/rg727/1950_1965_Training_Testing/Training/fold_5_neighbors/'+basin_id+'.csv',delimiter=",", dtype=np.float32, skiprows=1)
    ...
    for epoch in range(num_epochs):
        ...
        # SAVE MODEL
        torch.save(model.state_dict(),'/scratch/bcqp/rg727/1950_1965_Training_Testing/Training/fold_5_neighbors/model/lstm_nbr_ec_gl_train_seed_'+str(seed)+'_e'+str(epoch+1)+'.pth')
    
    
    # SAVE
    np.savetxt('/scratch/bcqp/rg727/1950_1965_Training_Testing/Training/fold_5_neighbors/model/loss_train_epoch_seed_'+str(seed)+'_ec.csv', loss_epoch, delimiter=',',header='train',comments='')
...
model = LSTM(num_input_feature, num_hidden_neuron, num_hidden_layer, num_output_feature, dropout_rate).to(device)      
model.load_state_dict(torch.load('/scratch/bcqp/rg727/1950_1965_Training_Testing/Training/fold_5_neighbors/model/lstm_nbr_ec_gl_train_seed_1000000_e30.pth'))
model.to(device)
...
for k, basin_id in enumerate(basin_list_valid):
    xy = np.loadtxt('/scratch/bcqp/rg727/1950_1965_Training_Testing/Testing/out_5_neighbors/'+basin_id+'.csv',delimiter=",", dtype=np.float32, skiprows=1)
    y = xy[:,[3]]
...
for k, basin_id in enumerate(basin_list_valid):
    ...
    with torch.no_grad():
        ...
        np.savetxt('/scratch/bcqp/rg727/1950_1965_Training_Testing/Testing/out_5_neighbors/OOS/lstm_nbr_pred_'+basin_id+'.csv', pred_obs, delimiter=',',header='pred,obs',comments='')
    ...
    np.savetxt('/scratch/bcqp/rg727/1950_1965_Training_Testing/Testing/out_5_neighbors/OOS/nse_validation_cv_check.csv', nse_valid, delimiter=',',header='test',comments='')
